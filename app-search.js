// Vognlogg – hash + søkbare nedtrekk via datalist
(function(){
  ...
  // ---- Søkbart datalist: fyll forslag og sett ID når brukeren velger ----
  function populateSearchLists(){
    ...
    peopleList.innerHTML = people.map(p=>`<option data-id="${p.id}" value="${p.name} (${p.number})${p.dept? ' – '+p.dept:''}"></option>`).join('');
    cartsList.innerHTML  = carts.map(c=>`<option data-id="${c.id}" value="${c.code} — ${c.type||''}${c.cap? ' · '+c.cap+' kg':''}${c.area? ' · '+c.area:''}${cartIsAvailable(c.id)?'':' [UTE]'}"></option>`).join('');
  }
  function wireSearchToSelect(){
    ...
    personSearch.addEventListener('input', ()=>{
      // Finn option med matchende value og sett skjult select til ID
  ...
  function populateRegForm(){
    populateSearchLists(); wireSearchToSelect();
  ...
  function attachRegHandlers(){
    ...
    $('#reg-form').addEventListener('submit', (e)=>{
      ...
      const note = $('#note').value||''; const dept = $('#dept').value||''; const project = $('#project').value||'';
      const errs = [];
      ...
      if (cart_id && action){ const available = cartIsAvailable(cart_id);
      ...
      const tx = load(KEYS.TX); const id = nextId(tx); const event_at = `${date} ${time}:00`;
  ...
  function renderPeople(){
    ...
    $$('#people-table [data-del-person]').forEach(btn=> btn.addEventListener('click', (ev)=>{
      ...
      renderPeople(); populateSearchLists(); // oppdatere søkeforslag
      populateRegForm(); populateReportFilters(); renderReport();
    }));
  }
  function attachPersonHandlers(){
    $('#person-form').addEventListener('submit',(e)=>{
      ...
      save(KEYS.PEOPLE, people);
      e.target.reset(); renderPeople(); populateSearchLists(); populateRegForm(); populateReportFilters(); renderReport();
    });
  }
  ...
  function renderCarts(){
    ...
    $$('#carts-table [data-del-cart]').forEach(btn=> btn.addEventListener('click', (ev)=>{
      ...
      renderCarts(); populateSearchLists(); populateRegForm(); populateReportFilters(); renderStatus(); renderReport();
    }));
  }
  function attachCartHandlers(){
    $('#cart-form').addEventListener('submit',(e)=>{
      ...
      save(KEYS.CARTS, carts);
      e.target.reset(); renderCarts(); populateSearchLists(); populateRegForm(); populateReportFilters(); renderStatus(); renderReport();
    });
  }
  ...
  function attachBackupHandlers(){
    ...
    $('#import-file').addEventListener('change', async (e)=>{
      ...
      try{ const payload=JSON.parse(text); if(payload.people && payload.carts && payload.transactions){
        ...
        populateSearchLists(); populateRegForm(); renderStatus(); populateReportFilters(); renderReport(); renderPeople(); renderCarts();
